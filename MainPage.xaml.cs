...
namespace CameraManualControls
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private async Task TakePhotoAsync()
        {
            ...
            if (cantidad == 1)
            {
                try
                {
                    ...
                    pictureFolder = await pictureFolder.CreateFolderAsync("prueba", CreationCollisionOption.OpenIfExists);

                    var Entradafile = await pictureFolder.GetFileAsync("entrada.txt");
                    ...
                    storageFolder = await storageFolder.CreateFolderAsync("prueba", CreationCollisionOption.OpenIfExists);
                    cont++;

                    // Create the file that we're going to save the photo to.
                    var file1 = await storageFolder.CreateFileAsync("sample(" + cont + ").jpg", CreationCollisionOption.ReplaceExisting);

                    // Update the file with the contents of the photograph.
                    await _mediaCapture.CapturePhotoToStorageFileAsync(ImageEncodingProperties.CreateJpeg(), file1);
                    Debug.WriteLine("Photo taken! Saving to " + file1.Path);

                    var photoOrientation = ConvertOrientationToPhotoOrientation(GetCameraOrientation());

                    await ReencodeAndSavePhotoAsync(stream, file1, photoOrientation);
                    ...
                    using (var stream1 = await file1.OpenAsync(FileAccessMode.Read))
                    {
                    ...
                    //crear fichero
                    await storageFolder.CreateFileAsync("datos.txt", CreationCollisionOption.ReplaceExisting);
                    var Samplefile = await storageFolder.GetFileAsync("datos.txt");
        ...
        /// </summary>
        /// <returns></returns>
        private async Task DeletedPicture()
        {
            try
            {
                    var bitmapSource = new SoftwareBitmapSource();
                if (cantidad == 1)
                {
                    ...
                    if (cont >= 1)
                    {
                        var storageFolder = KnownFolders.PicturesLibrary;
                        storageFolder = await storageFolder.CreateFolderAsync("prueba", CreationCollisionOption.OpenIfExists);
                        var file = await storageFolder.GetFileAsync("sample(" + cont + ").jpg");
                        cont--;
                        ...
                        if (cont <= 0)
                        {
                            ...
                            cont = 0;
                            await FileIO.WriteTextAsync(Samplefile, cont + "\n");
                            Debug.WriteLine("Foto NULL ultima foto");
                        }
                        else
                        {
                            file = await storageFolder.GetFileAsync("sample(" + cont + ").jpg");
                            await FileIO.WriteTextAsync(Samplefile, cont + "\n");
                            using (var stream = await file.OpenAsync(FileAccessMode.Read))
                            {
                                ...
                                await bitmapSource.SetBitmapAsync(softwareBitmapBGR8);
                                imageControl.Source = bitmapSource;
                                Debug.WriteLine("Foto Borrada y Muestra Ultima");
                            }
                        }
                    }
                }
